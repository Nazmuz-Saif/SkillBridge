...
def my_applications(request):
    applications = Application.objects.filter(freelancer=request.user).select_related('job').order_by('-applied_at')
    return render(request, 'jobs/my_applications.html', {'applications': applications})

@login_required
def saved_jobs(request):
    saved = SavedJob.objects.filter(freelancer=request.user).select_related('job').order_by('-saved_at')
...
def client_jobs(request):
    ...
    return render(request, 'jobs/client_jobs.html', {'jobs': jobs})
